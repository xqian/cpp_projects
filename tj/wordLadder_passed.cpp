// WordLadder.cpp : Defines the entry point for the console application.
//
...
public:
    vector<vector<string> > findLadders(string start, string end, unordered_set<string> &dict) {
        ...
        vector<unordered_set<int> > neighbor(vs.size(), unordered_set<int>());
		//LARGE_INTEGER cur;
		//LARGE_INTEGER cur1;
		/*
		//====================================================
		...
		cout << "Counter: " << cur.QuadPart << endl;
        calNeighbor(vs, neighbor);
		LARGE_INTEGER cur1;
		QueryPerformanceCounter(&cur1);
		cout << "CalNeighor cost: " << cur1.QuadPart - cur.QuadPart << endl;
		*/
		vector<bool> isNeighborCalculated(vs.size(), false);
		//====================================================
        int start_i;
		//QueryPerformanceCounter(&cur);
		//cout << "Counter: " << cur.QuadPart << endl;
		...
		//QueryPerformanceCounter(&cur1);
		//cout << "Find cost: " << cur1.QuadPart - cur.QuadPart << endl;
        if ( iter == vs.end()) return res;
        end_i = iter - vs.begin();
		//=====================================================
		...
		//QueryPerformanceCounter(&cur);
		//cout << "Counter: " << cur.QuadPart << endl;
        bfs(neighbor, start_i, end_i, distanceToDst, isNeighborCalculated,vs, m);
		//QueryPerformanceCounter(&cur1);
		//cout << "BFS cost: " << cur1.QuadPart - cur.QuadPart << endl;
		//====================================================
		...
		//QueryPerformanceCounter(&cur1);
		//cout << "DFS getpath cost: " << cur1.QuadPart - cur.QuadPart << endl;
        return res;
    }
...
private:
    ...
    void dfs(const vector<unordered_set<int> > & neighbor, 
         ...
         vector<int> &path, vector<int> & isVisited) {
            ...
            if ( cur == dst) {
                ...
                isVisited[cur] = 1;
                path.push_back(cur);
                for(auto iter=neighbor[cur].begin(); iter != neighbor[cur].end(); ++iter) {
                    dfs(neighbor, *iter, dst, bestPath, path, isVisited);                    
                }
                path.pop_back();
                isVisited[cur] = 0;
            }            
    ...
    void bfs(vector<unordered_set<int> > & neighbor, int src, int dst, vector<int> & distanceToDst, vector<bool> & isNeighborCalculated, 
				const vector<string> & vs, const unordered_map<string, int> & m ) {
        queue<int> q;
        q.push(dst);
        distanceToDst[dst] = 0;
        while( !q.empty()) {
            int t = q.front();
            q.pop();
			if ( t == src) return;//If want to calculate all paths, must return here when it pop
			if ( isNeighborCalculated[t] == false) {
				calNeighbor(neighbor, t, vs, m);
				isNeighborCalculated[t] = true;
			}
            for(auto iter = neighbor[t].begin(); iter != neighbor[t].end(); ++iter) {
                if ( distanceToDst[*iter] == -1) {
                    distanceToDst[*iter] = distanceToDst[t]+1;                    
                    q.push(*iter); //BUG forget push                  
                }
    ...
    void dfsGetPath(const vector<unordered_set<int> > & neighbor, int cur, int dst, 
         ...
         for(auto iter=neighbor[cur].begin(); iter != neighbor[cur].end(); ++iter) {
            if ( distanceToDst[cur] - 1 == distanceToDst[*iter] ) {
                dfsGetPath(neighbor, *iter, dst, distanceToDst, vs, res, path);
            }
         }
         path.pop_back();         
    }
...
Countrary to what we thought, CalNeighbor cost most!!!

Counter: 16095669912
CalNeighor cost: 4289139
Counter: 16099963577
Find cost: 4051
Counter: 16099969488
BFS cost: 15466
Counter: 16099986744
DFS getpath cost: 6154

CalNeighbor:
N * N /2 * L

If for each string, calculate all possible neighbor
